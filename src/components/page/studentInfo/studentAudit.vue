...
export default {
  ...
  methods: {
    ...
    updateStudents() {
      const currThis = this
      currThis.$axios.post('http://47.114.1.9/traffic/studentinfo/shenHeStudent', {
        params: {
          id: currThis.stuid,
          status : 1,
          headImgStatus: currThis.headImgStatus,
        }
      })
      .then(res => currThis.$emit('AuditSuccessClick', res.data))
