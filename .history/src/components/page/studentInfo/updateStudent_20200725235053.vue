<template>
  <div>
    <el-form :model="student" label-position="left" :rules="rules" ref="stus" label-width="120px" class="demo-ruleForm">
      ...
      <el-form-item label="学员头像" prop="headImg">
         <el-upload
        ...
        >
          <img  v-if="student.headImg" :src="student.headImg" />
      ...
      <el-form-item>
        <el-button type="primary" @click="updateStudents('stus')">立即创建</el-button>
  ...
  methods: {
    ...
    updateStudents(stus) {
      this.$refs[stus].validate((valid) => {
        if (valid) {
          const currThis = this
            currThis.student.inductions = currThis.$moment(currThis.student.inductions).format('YYYY-MM-DDTHH:mm:ss')
            currThis.student.driverOverTimes = currThis.$moment(currThis.student.driverOverTimes).format('YYYY-MM-DDTHH:mm:ss')
            currThis.student.busCarefulTimes = currThis.$moment(currThis.student.busCarefulTimes).format('YYYY-MM-DDTHH:mm:ss')
            currThis.student.createTimes = currThis.$moment(currThis.student.createTimes).format('YYYY-MM-DDTHH:mm:ss')
