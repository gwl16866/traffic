...
<script>
	export default {
		...
		methods: {
			...
			// 重置表单并刷新页面
			resetForm() {
				console.log(this.student.realName)
				console.log(this.student.createTime)
				console.log(this.student.endTime)
				if(!this.student.realName == undefined ){
					this.student.realName = ''
				}else if(!this.student.createTime == undefined ){
					this.student.createTime = this.$options.data().createTime
				}else if( !this.student.endTime == undefined ){
					this.student.endTime = this.$options.data().endTime
				}
				this.queryAllStudent()
			}
		}
