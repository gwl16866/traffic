<template>
	<div>
      ...
      <el-table-column label="操作" align="center" min-width="20%">
        <template slot-scope="scope">
          <el-button v-if="scope.row.status === 1" type="primary" round @click="updateUser(scope.row)">编辑用户</el-button>
					<el-button v-if="scope.row.status !== 1" type="primary" round @click="updateUser(scope.row)">激活</el-button>
        </template>
      </el-table-column>
    </el-table>
		<!-- --------------------分页开始------------------------------------------------------------------------------ -->
    ...
    <el-dialog title="编辑学员" :visible.sync="updateStudentVisible">
      <activityCommodity v-if="updateStudentVisible" :seckillid="seckillId" />
    </el-dialog>
    <!-- --------------------操作弹出框结束------------------------------------------------------------------------ -->
	</div>
</template>
...
<script>
import updatestudent from '@/views/components/operate/activityCommodity'
	export default {
		data() {
			return {
				student: {status:1},
