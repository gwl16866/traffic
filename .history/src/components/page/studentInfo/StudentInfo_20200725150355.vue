	...
	export default {
		...
		methods: {
			...
			queryAllStudent(){
				...
				let createTime = ''
				let endTimes = ''
				if (currThis.student.createTime != null && currThis.student.endTimes != null) {
					createTime = currentThis.$moment(currThis.student.createTime).format('YYYY-MM-DD')
					endTimes = currentThis.$moment(currThis.student.endTime).format('YYYY-MM-DD')
				}
				currentThis.$axios.get('http://localhost:8081/studentinfo/queryAllStudentInFo',{
					params: {
						realName: currThis.student.realName,
						createTime: currThis.selectSeckill.seckillStatus,
						startTimes: createTime,
						endTimes: endTimes,
