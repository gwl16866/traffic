<template>
	<div>
      ...
      <el-form ref="student" :inline="true" :model="studentList" class="demo-form-inline">
        <el-form-item label="学员姓名">
          <el-input v-model="studentList.realName" placeholder="用户姓名" width="10px"/>
        </el-form-item>

				<el-form-item label="录入时间">
					<el-col :span="11">
						<el-date-picker v-model="studentList.createTime" type="date" placeholder="选择日期" />
					</el-col>
        ...
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.cardId }}
        </template>
      </el-table-column>

			<el-table-column label="联系电话" align="center" min-width="30%">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.linkNum }}</span>
        </template>
      </el-table-column>

			<el-table-column label="车牌号码" align="center" min-width="30%">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.busNum }}</span>
        </template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.status === 0 ?'禁用': scope.row.status===1?'激活':'离职'}}</span>
        </template>
      </el-table-column>
