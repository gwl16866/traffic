<template>
      ...
      <el-table-column label="操作" align="center" min-width="20%">
        <template slot-scope="scope">
          <el-button v-if="scope.row.status === 1" type="primary" round @click="updateStudent(scope.row)">编辑用户</el-button>
					<el-button v-if="scope.row.status !== 1" type="primary" round @click="updateStatus(scope.row)">激活</el-button>
        </template>
      </el-table-column>
    </el-table>
		<!-- --------------------分页开始------------------------------------------------------------------------------ -->
    ...
    <el-dialog title="编辑学员" :visible.sync="updateStudentVisible">
      <updateStudent v-if="updateStudentVisible"  />
    </el-dialog>

		<el-dialog title="提示"	:visible.sync="updateStatus"	width="30%">
			<span>你确定要激活账号吗？</span>
			<span slot="footer" class="dialog-footer">
				<el-button @click="closeUpdateStatus = false">取 消</el-button>
