...
<div>
    ...
    <el-table :data="tableData" >
        ...
        <el-table-column prop="learnType" width="270"></el-table-column>
        <el-table-column prop="project,learnTime" width="230">
           <template slot-scope="scope">
            培训科目:{{scope.row.project}}
           <div> 培训时长:{{scope.row.learnTime}}</div>
          </template>
        </el-table-column>

        <el-table-column width="250" >
            <template slot-scope="scope">
                <div>当前进度:（{{scope.row.ok}}/{{scope.row.allproper}}学员）</div>
              <el-progress :text-inside="true" :stroke-width="12" :percentage="scope.row.bfb"></el-progress>
            </template>
        </el-table-column>
    ...
    methods:{
      ...
      //获取当前年中的当前月
        ...
        async selectTableInfo(){
            const that = this
            this.$axios.get('http://localhost:8081/trainProgress/queryThemTable.do',{
                params:{
